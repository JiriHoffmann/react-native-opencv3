...
class CvCamera extends Component {
  constructor(props) {
    ...
    this.state = {
      cameraPermissed:
        Platform.OS === 'ios' ? true : PermissionsAndroid.PERMISSIONS.CAMERA,
    };
  }
  componentDidMount = () => {
  ...
  async requestCameraPermissions() {
    try {
      ...
      if (granted === PermissionsAndroid.RESULTS.GRANTED) {
        const retJson = await NativeModules.CvCameraModule.initCamera(
          findNodeHandle(this),
        );
  ...
  async takePicture(filename) {
    ...
    if (Platform.OS === 'android') {
      return await NativeModules.CvCameraModule.takePicture(
        pictureOptions,
        findNodeHandle(this),
      );
    } else {
      return await NativeModules.CvCameraView.takePicture(
        pictureOptions,
        findNodeHandle(this),
      );
    }
  ...
  startRecording(filename) {
    ...
    if (Platform.OS === 'android') {
      NativeModules.CvCameraModule.startRecording(
        pictureOptions,
        findNodeHandle(this),
      );
    } else {
      NativeModules.CvCameraView.startRecording(
        pictureOptions,
        findNodeHandle(this),
      );
    }
  ...
  async stopRecording() {
    if (Platform.OS === 'android') {
      return await NativeModules.CvCameraModule.stopRecording(
        findNodeHandle(this),
      );
    } else {
      return await NativeModules.CvCameraView.stopRecording(
        findNodeHandle(this),
      );
    }
  }
...
const useCascadeOnImage = (cascade, image) => {
  return RNOpencv3.useCascadeOnImage(cascade, image);
};
